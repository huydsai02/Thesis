...
class BeamSearchDecoderCTC:
    ...
    def __init__(
        ...
        self.DEFAULT_HOTWORD_SCORER = HotwordScorer.build_scorer(None, weight=DEFAULT_HOTWORD_WEIGHT)
    ...
    def _get_lm_beams(
        ...
        new_beams = []
        for text, next_word, word_part, last_char, frame_list, frames, logit_score in beams:
            # print(f"text: {text}, next_word: {next_word}, word_part: {word_part}, last_char: {last_char}, frame_list: {frame_list}, frames: {frames}, logit_score: {logit_score}")
            # fast token merge
            new_text = _merge_tokens(text, next_word)
            if new_text not in cached_lm_scores:
                ...
                # print(f"new_text: {new_text}, raw_lm_score: {raw_lm_score}, lm_hw_score: {lm_hw_score}, hotword_scorer: {hotword_scorer.score(new_text)}")
                cached_lm_scores[new_text] = (lm_hw_score, raw_lm_score, end_state)
            lm_score, _, _ = cached_lm_scores[new_text]

            # print(f"w: {word_part}, cached_partial_token_scores: {cached_partial_token_scores}")
            if len(word_part) > 0:
    ...
    def partial_decode_logits(
        ...
        cached_p_lm_scores: Dict[str, float] = None,
        beams = None, 
        processed_frames=0,
        is_end=False
    ) -> List[OutputBeam]:
        ...
        if hotwords is None and hotword_weight == DEFAULT_HOTWORD_WEIGHT:
            hotword_scorer = self.DEFAULT_HOTWORD_SCORER
        else:
        ...
        # print("BEAMS:", beams)
        # print("CACHED_P_LM_SCORES:", cached_p_lm_scores)
        # print("CACHED_LM_SCORES:", cached_lm_scores)
        ...
        if not is_end:
            return beams, cached_lm_scores, cached_p_lm_scores
        # print("HELLO END")
