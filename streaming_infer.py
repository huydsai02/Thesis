...
model = Wav2Vec2SmallNonStreamingForCTC.from_pretrained(
    ...
    mem_length=MEM_LENGTH, num_frames=NUM_FRAME,
...
def streaming_asr(path):
    ...
    if sr != 16000:
        ...
        path = os.path.join(os.path.dirname(__file__), "streaming_temp.wav")
    ...
    while not is_end:
        if current_frame == 0:
            ...
            signal = np.frombuffer(data, dtype='int16') * 0.5**15
            audio_input = np.concatenate([audio_input, signal])[OTHER_CHUNK:]
